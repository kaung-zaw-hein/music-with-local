<template>
  <div>
    <div class="mx-auto">
    <h1 class="ml-4 text-3xl text-white border-b border-gray-300">By {{this.$route.params.pp}}</h1>
  </div>
  <ul class="mx-auto">
    ...
    v-for="song in songs" :key="song.docID">
          ...
          <div class="text-lg text-left text-white ">
            <router-link exact-active-class="no-active"  :to="{name:'Uploader',params:{pp:song.display_name}}" class="ml-32 -mt-4 text-sm font-bold text-white">{{ $t('song.uploadedby') }} {{song.display_name}}</router-link>
          </div>
          <div class="text-lg text-left text-white ">
            <router-link  :to="{ name: 'Genre', params: { tags: song.genre }}" class="ml-32 -mt-4 text-sm font-bold text-white">{{song.genre}}</router-link>
          </div>
  ...
  methods: {
    async getSong(){
      const snapshots =  await songsCollection
        .where("display_name","==",this.$route.params.pp)
        .get();
